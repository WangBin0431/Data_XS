...
ylabel('Mean value of error (px)');
legend('Aliased data','Separate data');
axis([0,size(rate,2)/20,0.8*min(min(p3*8),min(psum*8)),1.2*max(max(p3*8),max(psum*8))]);
...
title(['Interval time ',num2str(t2-t1),'ns']);
xlabel('Q_t_o_t_a_l(2)/Q_t_o_t_a_l(1)');
ylabel('Standard deviation of error (px)');
legend('Aliased data','Separate data');
axis([0,size(rate,2)/20,0.8*min(min(p31*8),min(psum1*8)),1.2*max(max(p31*8),max(psum1*8))]);